$(document).ready(function () {
  $.ajax({
    ...
    if (status) {
      ...
      alert(false, "Problem in products search (" + data.message + ")");
    }
  });
  ...
  $(".row")
    ...
    .on("click", ".btnSubmit", function () {
      ...
      $.ajax({
        ...
        if (status) {
          console.log(true, "Success product insert");
          alert(true, "Success product insert");
          $("#frmProduct")[0].reset();
        } else {
          console.log(
            false,
            "Problem in product insert (" + data.message + ")"
          );
          alert(false, "Problem in product insert (" + data.message + ")");
          $("#frmProduct")[0].reset();
        }
      });
